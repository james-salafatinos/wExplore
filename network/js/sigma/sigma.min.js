//Copied from http://sigmajs.org/js/sigma.min.js
...
(function () {
  ...
  function m() {
      ...
      ((a instanceof Array ? a : [a]) || []).forEach(function (a) {
        if (d.nodesIndex[a]) {
          ...
          d.edges = d.edges.filter(function (c) {
            if (c.source.id == a)
              return (
                delete d.edgesIndex[c.id],
                c.target.degree--,
                c.target.inDegree--,
                !1
              );
            return c.target.id == a
              ? (delete d.edgesIndex[c.id],
                c.source.degree--,
                c.source.outDegree--,
                !1)
    ...
    this.rescale = function (a, b, c, g) {
      ...
      var f =
          "outside" == d.p.scalingMode
            ? Math.max(a / Math.max(k - l, 1), b / Math.max(p - s, 1))
            : Math.min(a / Math.max(k - l, 1), b / Math.max(p - s, 1)),
      ...
      var f =
        ...
        j,
        n;
      !d.p.maxNodeSize && !d.p.minNodeSize
        ? ((j = 1), (n = 0))
        : d.p.maxNodeSize == d.p.minNodeSize
        ? ((j = 0), (n = d.p.maxNodeSize))
        : ((j = (d.p.maxNodeSize - d.p.minNodeSize) / h),
          (n = d.p.minNodeSize));
      var x, m;
      !d.p.maxEdgeSize && !d.p.minEdgeSize
        ? ((x = 1), (m = 0))
        : ((x =
            d.p.maxEdgeSize == d.p.minEdgeSize
              ? 0
              : (d.p.maxEdgeSize - d.p.minEdgeSize) / o),
          (m = d.p.minEdgeSize));
      c &&
        d.nodes.forEach(function (c) {
          c.displaySize = c.size * j + n;
          if (!c.fixed)
            (c.displayX = (c.x - (k + l) / 2) * f + a / 2),
    ...
    this.setBorders = function () {
      d.borders = {};
      d.nodes.forEach(function (a) {
        d.borders.minX = Math.min(
          void 0 == d.borders.minX
            ? a.displayX - a.displaySize
            : d.borders.minX,
          a.displayX - a.displaySize
        );
        d.borders.maxX = Math.max(
          void 0 == d.borders.maxX
            ? a.displayX + a.displaySize
            : d.borders.maxX,
          a.displayX + a.displaySize
        );
        d.borders.minY = Math.min(
          void 0 == d.borders.minY
            ? a.displayY - a.displaySize
            : d.borders.minY,
          a.displayY - a.displaySize
        );
        d.borders.maxY = Math.max(
          void 0 == d.borders.maxY
            ? a.displayY - a.displaySize
            : d.borders.maxY,
          a.displayY - a.displaySize
        );
  ...
  function j(b) {
    ...
    function g(b, c, g) {
      if (
        ...
        (m = a.p.directZooming ? 1 - (w ? a.p.zoomDelta : a.p.dragDelta) : 0),
        a.ratio != h || a.stageX != j || a.stageY != k)
      )
        d(),
    ...
    function d() {
      ...
      a.ratio = c * (1 - b) + h * b;
      w
        ? ((a.stageX = j + ((a.stageX - j) * a.ratio) / c),
          (a.stageY = k + ((a.stageY - k) * a.ratio) / c))
        : ((a.stageX = i * (1 - b) + j * b), (a.stageY = o * (1 - b) + k * b));
      ...
      if (1 <= m)
        window.clearInterval(a.interpolationID),
          ...
          w
            ? ((a.ratio = h),
              (a.stageX = j + ((a.stageX - j) * a.ratio) / b),
              (a.stageY = k + ((a.stageY - k) * a.ratio) / b))
            : ((a.stageX = j), (a.stageY = k)),
    ...
    document.addEventListener(
      ...
      function (b) {
        if (a.p.mouseEnabled && a.isMouseDown)
          (a.isMouseDown = !1),
            a.dispatch("mouseup"),
            (i != a.stageX || o != a.stageY) &&
              g(
                a.stageX + a.p.inertia * (a.stageX - p),
                a.stageY + a.p.inertia * (a.stageY - n)
              ),
            b.preventDefault ? b.preventDefault() : (b.returnValue = !1);
      },
      !0
  ...
  function n(b, l, g, d, a, f, c) {
    function i(a) {
      ...
      sigma.tools.drawRoundRect(
        b,
        Math.round(a.displayX - c / 2 - 2),
        Math.round(a.displayY - c / 2 - 2),
        Math.round(
          b.measureText(a.label).width + 1.5 * a.displaySize + c / 2 + 4
        ),
        Math.round(c + 4),
        Math.round(c / 2 + 2),
        "left"
    ...
    function o(a) {
      ...
      return (
        !a.hidden &&
        a.displayX + a.displaySize > -j / 3 &&
        a.displayX - a.displaySize < (4 * j) / 3 &&
        a.displayY + a.displaySize > -k / 3 &&
        a.displayY - a.displaySize < (4 * k) / 3
      );
    }
    sigma.classes.Cascade.call(this);
    var h = this;
    ...
    this.task_drawEdge = function () {
      for (
        var b = a.edges.length, c, d, g = 0;
        g++ < h.p.edgesSpeed && h.currentEdgeIndex < b;
  ...
  function u(b, l) {
    function g() {
      sigma.chronos
        .removeTask("node_" + c.id, 2)
        .removeTask("edge_" + c.id, 2)
        .removeTask("label_" + c.id, 2)
        .stopTasks();
      return c;
    }
    function d(a, b) {
    ...
    function a() {
      c.p.drawHoverNodes &&
        (c.graph.checkHover(c.mousecaptor.mouseX, c.mousecaptor.mouseY),
        c.graph.nodes.forEach(function (a) {
    ...
    this.mousecaptor
      .bind("drag interpolate", function () {
        c.draw(
          c.p.auto ? 2 : c.p.drawNodes,
          c.p.auto ? 0 : c.p.drawEdges,
          c.p.auto ? 2 : c.p.drawLabels,
          !0
        );
      })
      ...
      .bind("mousedown mouseup", function (a) {
        ...
        b.length &&
          c.dispatch("mousedown" == a.type ? "downnodes" : "upnodes", b);
      })
      .bind("move", function () {
        c.domElements.hover
          .getContext("2d")
          .clearRect(
            0,
            0,
            c.domElements.hover.width,
            c.domElements.hover.height
          );
        a();
        A();
      });
    ...
    this.draw = function (a, b, d, l) {
      ...
      c.graph.translate(
        c.mousecaptor.stageX,
        c.mousecaptor.stageY,
        c.mousecaptor.ratio,
        0 < a,
        0 < b
      );
      ...
      for (var f in c.domElements)
        "canvas" == c.domElements[f].nodeName.toLowerCase() &&
          (void 0 == l[f] || 0 <= l[f]) &&
          c.domElements[f]
            .getContext("2d")
            .clearRect(0, 0, c.domElements[f].width, c.domElements[f].height);
      ...
      l = !1;
      if (a)
        if (1 < a) for (; c.plotter.task_drawNode(); );
        else
          sigma.chronos.addTask(c.plotter.task_drawNode, "node_" + c.id, !1),
            (l = !0),
            (f = "node_" + c.id);
      if (d)
        if (1 < d) for (; c.plotter.task_drawLabel(); );
        else
          f
            ? sigma.chronos.queueTask(
                c.plotter.task_drawLabel,
                "label_" + c.id,
                f
              )
            ...
            (f = "label_" + c.id);
      if (b)
        if (1 < b) for (; c.plotter.task_drawEdge(); );
        else
          f
            ? sigma.chronos.queueTask(
                c.plotter.task_drawEdge,
                "edge_" + c.id,
                f
              )
            : sigma.chronos.addTask(
    ...
    this.resize = function (a, b) {
      ...
      void 0 != a && void 0 != b
        ...
        : ((c.width = c.domRoot.offsetWidth),
          (c.height = c.domRoot.offsetHeight));
      if (d != c.width || g != c.height) {
        for (var f in c.domElements)
          c.domElements[f].setAttribute("width", c.width + "px"),
    ...
    this.refresh = function () {
      c.domElements.hover
        .getContext("2d")
        .clearRect(0, 0, c.domElements.hover.width, c.domElements.hover.height);
      a();
      A();
      return c;
    };
    ...
    this.zoomTo = function (g, d, a) {
      ...
      a == b.mousecaptor.ratio
        ? b.mousecaptor.interpolate(
            g - b.width / 2 + b.mousecaptor.stageX,
            d - b.height / 2 + b.mousecaptor.stageY
          )
        : b.mousecaptor.interpolate(
            (a * g - (b.mousecaptor.ratio * b.width) / 2) /
              (a - b.mousecaptor.ratio),
            (a * d - (b.mousecaptor.ratio * b.height) / 2) /
              (a - b.mousecaptor.ratio),
            a
          );
      return f;
      ...
      g.edges &&
        g.edges.forEach(function (a) {
          (validID = a.source && a.target && a.id) &&
            (!d || !b.graph.edgesIndex[a.id]) &&
            f.addNode(a.id, a.source, a.target, a);
        });
      return f;
    };
  ...
  sigma.chronos = new (function () {
    ...
    function g() {
      ...
      if (!t[z].task()) {
        ...
        v = v.filter(function (b) {
          b.taskParent == a && t.push({ taskName: b.taskName, task: b.task });
    ...
    function o(a) {
      if (void 0 != q[a.content.taskName])
        q[a.content.taskName].del || !q[a.content.taskName].condition()
          ? delete q[a.content.taskName]
          : (q[a.content.taskName].on = !1),
          0 == c(!0) && j();
    }
    sigma.classes.EventDispatcher.call(this);
    var h = this,
    ...
    this.removeTask = function (b, c) {
      if (void 0 == b)
        ...
        t = t.filter(function (a) {
          return ("string" == typeof b ? a.taskName == b : a.task == b)
            ? ((d = a.taskName), !1)
            : !0;
        });
        0 < c &&
  ...
  sigma.tools.drawRoundRect = function (b, f, g, d, a, i, c) {
    var i = i ? i : 0,
      ...
      m =
        i &&
        (0 <= j.indexOf("topright") ||
          0 <= j.indexOf("top") ||
          0 <= j.indexOf("right")),
      h =
        i &&
        (0 <= j.indexOf("bottomleft") ||
          0 <= j.indexOf("bottom") ||
          0 <= j.indexOf("left")),
      j =
        i &&
        (0 <= j.indexOf("bottomright") ||
          0 <= j.indexOf("bottom") ||
          0 <= j.indexOf("right"));
    b.moveTo(f, g + i);
    c ? b.arcTo(f, g, f + i, g, i) : b.lineTo(f, g);
    m
      ? (b.lineTo(f + d - i, g), b.arcTo(f + d, g, f + d, g + i, i))
      : b.lineTo(f + d, g);
    j
      ? (b.lineTo(f + d, g + a - i), b.arcTo(f + d, g + a, f + d - i, g + a, i))
      : b.lineTo(f + d, g + a);
    h
      ? (b.lineTo(f + i, g + a), b.arcTo(f, g + a, f, g + a - i, i))
      : b.lineTo(f, g + a);
    b.lineTo(f, g + i);
  };
